...
7. **framer-motion 会把 `filter: blur(0px)` 留在每个单元的内联样式上**。动画播完后它视觉上等于 `none`，但 Safari 只要看到元素上还有 filter 就会给它建合成层，并在那个层里显示出一块和文字等大的灰色矩形——逐字拆得越散灰块越多（contact 标题上每个词一块）。所以播放一结束就给容器加 `.reveal-done`，由 CSS 用 `filter: none` 摘掉它（该规则必须放在 `.reveal-play` 之后，按源顺序取胜）。
